...
test('E2E test', { tag: '@regression' }, async ({ browser }) => {
    ...
    await Page.getByRole('alert', { name: 'Product Added To Cart' }).waitFor({ state: 'visible' })
    const labelCount = await cart.locator('label').textContent()

    expect(parseInt(labelCount)).toBeGreaterThan(0)
    await Page.getByRole('alert', { name: 'Product Added To Cart' }).waitFor({ state: 'hidden' })
    await cart.click()
    expect(Page.url()).toContain('/cart')

    const subTotal = await Page.locator('.totalRow', { hasText: 'Subtotal' }).locator('span.value')
        .textContent().then(e => e.replace('$', ''))

    const total = await Page.locator('.totalRow', { hasText: /^Total/i })
        .locator('span.value').textContent().then(e => e.replace('$', ''))
    ...
    const orderId = await Page.locator('td.em-spacer-1 label.ng-star-inserted')
        .textContent().then(e => e.replaceAll("|", "").trim())
    ...
    for (let i = 0; await table.count() >= i; i++) {
        ...
        const bool = orderId.includes(await table.locator('th').nth(i).textContent())
